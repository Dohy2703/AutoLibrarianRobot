...
# modified : function { compute_ratio_and_resize(img,width,height,model_height) }
...
def compute_ratio_and_resize(img,width,height,model_height):
    ...
    # print('model_height : ', model_height, 'model_width : ', model_height*ratio)

    if ratio<1.0:
        ratio = calculate_ratio(width,height)

        # img = cv2.resize(img,(model_height,int(model_height*ratio)), interpolation=Image.Resampling.LANCZOS)
        # img = cv2.resize(img, (model_height, int(model_height * ratio)), interpolation=cv2.INTER_CUBIC)  # 육안으로 볼 때 성능 가장 좋음
        img = cv2.resize(img, (int(1.5*model_height), int(1.5*model_height * ratio)),
                         interpolation=cv2.INTER_CUBIC)
    else:
        # img = cv2.resize(img,(int(model_height*ratio),model_height),interpolation=Image.Resampling.LANCZOS)
        # img = cv2.resize(img, (int(model_height * ratio), model_height), interpolation=cv2.INTER_CUBIC)
        img = cv2.resize(img, (int(1.5*model_height * ratio), int(1.5*model_height)), interpolation=cv2.INTER_CUBIC)
    ...
    # 2
    # blr = cv2.GaussianBlur(img, (0, 0), 4) # 표준편차 2, 필터 크기는 자동 설정
    # a = 2.0 # 샤프닝 정도 결정하는 변수
    # img = np.clip((1+a)*img - a * blr, 0, 255).astype(np.uint8)
    ...
    # img = cv2.addWeighted(img, 4, smoothed, -3, 0)
...
def get_image_list(horizontal_list, free_list, img, model_height = 64, sort_output = True):
    ...
    for box in free_list:
        ...
        # print(box)  # 아무것도 안나옴
        # cv2.imshow('cr img', crop_img) # 아무것도 안나옴
    ...
    for box in horizontal_list:
        ...
        # print('img list', image_list[0])
        # cv2.imshow('cp img', crop_img)
        # cv2.imwrite('/home/kdh/Desktop/crop/INTER_LANCZOS4/'+str(idx_)+'.png', crop_img)
        # idx_ = idx_ + 1
        # cv2.waitKey(0)
