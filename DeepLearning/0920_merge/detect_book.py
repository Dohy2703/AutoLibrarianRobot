...
class DetectBook(labelReader):  # 책의 특징점을 찾아서 tf를 만드는 클래스
    ...
    def ocr(self, word='6229', save_points=True, resize=False, visualize=False) -> tuple:
        ...
        if (len(find_label) == 0):
            ...
            detect_one_book_mask = False  # 하나의 책 마스크를 검출하게 됩니다.

            if detect_one_book_mask :
                (b_ch, g_ch, r_ch) = cv2.split(self.color_image)
                book_label_mask = book_label_mask/255

                # label_masks *= self.gray_image
                b_label_masks = book_label_mask.copy()
                g_label_masks = book_label_mask.copy()
                r_label_masks = book_label_mask.copy()

                b_label_masks *= b_ch
                g_label_masks *= g_ch
                r_label_masks *= r_ch

                # book_label_masks_color = cv2.merge((b_label_masks, g_label_masks, r_label_masks))
                book_label_masks_color = cv2.merge([b_label_masks, g_label_masks, r_label_masks])
                cv2.imwrite('/home/kdh/Desktop/material.png', book_label_masks_color)
                cv2.waitKey(0)
