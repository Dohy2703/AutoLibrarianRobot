...
class TCP:  # 통신 클래스 (파이썬 <-> Matlab) 파이썬 : client
    ...
    def receive(self):  # 반환값 = int
        if self.available:
            try:
                data = self.client_socket.recv(10).decode('UTF-8')
                # print(data)
                # print("1",data[0])
                # print("2",data[len(data)-2])
                # print(f'{data}', 'len = ', len(data), type(data))
                if data[0] == '/':
                    if data[len(data)-2] == '/':
                        # data = data[1:-1]
                        self.recv_data = data[1] # 문자열로 들어옴
                        # self.recv_data = int(data)
