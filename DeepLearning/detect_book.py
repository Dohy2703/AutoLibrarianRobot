...
class DetectBook:  # 책의 특징점을 찾아서 tf를 만드는 클래스
    ...
    def ocr(self, word='6229', resize=False) -> tuple:
        ...
        # result = reader.readtext(self.label_masks, slope_ths=0.3)
        result = reader.readtext(output1, slope_ths=0.3)
        temp_idx = 0
        ...
        for i in range(len(result)):
            ...
            if word in result[i][1]:
                ...
                # 라벨지를 찾았을 때
                self.label_tracking_idx = self.mask_label_idx[temp_idx]  # 찾은 라벨지의 인덱스
                self.LC_point = self.mask_centers[self.label_tracking_idx]  # 찾은 라벨지의 중심점

                label_contour = self.contours[self.label_tracking_idx]  # 찾은 라벨지의 minAreaRect bbox
                ...
                if resize:
                    find_label = cv2.resize(self.masks[self.label_tracking_idx].byte().cpu().numpy()*255,
                                            (self.origin_w, self.origin_h))
                else:
                    find_label = self.masks[self.mask_label_idx[temp_idx]].byte().cpu().numpy() * 255

                # cv2.circle(find_label, (int(self.LC_point[0]), int(self.LC_point[1])), 2, (0, 0, 0), 2) # 확인용
                ...
                for idx_, item in enumerate(self.book_list):
                    ...
                    if (numerator / denominator) < min_:
                        min_ = (numerator / denominator)
                        min_idx = idx_

                # 책을 찾았을 때
                self.book_tracking_idx = self.book_idx[min_idx]  # 찾은 책의 인덱스
                self.BC_point = self.mask_centers[self.book_tracking_idx]  # 찾은 책의 중심점

                book_contour = self.contours[self.book_tracking_idx]  # 찾은 책의 minAreaRect bbox
                ...
                if resize :
                    find_book = cv2.resize(self.masks[self.book_tracking_idx].byte().cpu().numpy()*255,
                                       (self.origin_w, self.origin_h))
                else :
    ...
    def center_book(self):
        ...
        center_book_idx = self.book_idx[B_min_idx]
